...
function get_data(file) # Reads, processes, and splits our data from the vlm file into sets for training and testing
    ...
    num_inputs = 6 # This is our 6 inputs, alpha, root, x1, x2, x3, x4
...
function train()
    ...
    training_vars, training_vlm, test_vars, test_vlm, vlm_norm = get_data("vlm_neural_net/vlm_alpha_data_file_3.data")
    outputs = 48 # <<<<<<<<<<<<<<<<<<< MUST EDIT IF N_OUTPUTS CHANGES
    #region Model Definition
    println("Defining Model")
    model = Lux.Chain(
        Lux.Dense(6 => 30,relu),
        Lux.Dense(30 => 200,relu),
        Lux.Dense(200 => 200,relu),
        Lux.Dense(200 => 200,relu),
        Lux.Dense(200 => 200,relu),
        Lux.Dense(200 => outputs) 
    )
