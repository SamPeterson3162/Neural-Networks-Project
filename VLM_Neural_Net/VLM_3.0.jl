...
function main()
    ...
    data_lst = zeros(6+n_edges, n)
    for i in 1:n
        a = input_lst[1,i]
        x1, x2, x3, x4 = input_lst[3:6,i]
        cf, root = analyze_system(a, x1, x2, x3, x4)
        ...
        data_lst[n_edges + 2:end,i] .= input_lst[2:6,i] .* root
    end
    # Write to file
    output_file = "vlm_neural_net/vlm_alpha_data_file_3.data"
    delimiter = ' ' 
